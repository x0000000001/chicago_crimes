...
def get_figure(data):
    ...
    fig = px.line(data, x="Year", y="Annual", color="Primary Type", title="Number of crimes per year")
    fig.update_traces(hovertemplate=get_hover_template("Annual"))
    for trace in fig.data:
        trace.customdata = np.stack([np.full(len(trace.y), "Annual"), np.full(len(trace.y), trace.name)], axis=-1)

    # Add traces for Cumulative
    for primary_type in data["Primary Type"].unique():
        df = data[data["Primary Type"] == primary_type]
        customdata = np.stack([np.full(len(df), "Cumulative"), np.full(len(df), primary_type)], axis=-1)
        fig.add_trace(go.Scatter(x=df["Year"], y=df["Cumulative"], mode="lines", visible=False, name = primary_type,
                                hovertemplate=get_hover_template("Cumulative"), customdata=customdata, legendgroup=primary_type))
    ...
    for trace in fig.data:
        if trace.customdata[0][0] == "Annual":
            max_annual = max(max_annual, max(trace.y))
        else:
            max_cumulative = max(max_cumulative, max(trace.y))
...
def process_data(data):
    ...
    for year in data["Year"].unique():
        for primary_type in data["Primary Type"].unique():
            if primary_type not in data[data["Year"] == year]["Primary Type"].values:
                data = pd.concat([data, pd.DataFrame({"Year": [year], "Primary Type": [primary_type], "Annual": [0]})])
    ...
    if "OTHER OFFENSE" in data["Primary Type"].unique():
        data.loc[data["Primary Type"] == "OTHER OFFENSE", "Annual"] += other_offense["Annual"].values
    else:
